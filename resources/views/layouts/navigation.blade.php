        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center group">
                    <a href="{{ route('dashboard') }}" class="transition-transform duration-300 hover:scale-105">
                        <x-application-logo class="block text-2xl text-indigo-600 transition-colors duration-300 hover:scale-105" />
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->userRole->role === 'Student' || Auth::user()->userRole->role === 'Old Student')
                    <x-nav-link :href="route('student.history')" :active="request()->routeIs('student.history')" class="text-gray-300 hover:text-white">
                        {{ __('History') }}
                    </x-nav-link>
                    @endif
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')" class="hover:bg-purple-500/10 transition-colors duration-200">
                            {{ __('Profile') }}
                        </x-dropdown-link>
        ...
        <div class="pt-4 pb-1 border-t border-white/10">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('logout')" class="text-gray-300 hover:text-white hover:bg-white/5"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
